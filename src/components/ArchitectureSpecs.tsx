...
export function ArchitectureSpecs({ onBack }: Props) {
  return (
    <div className="relative min-h-[100dvh] bg-slate-50 text-slate-900 font-sans selection:bg-slate-200 flex flex-col">
      ...
      <main className="max-w-[1400px] w-full mx-auto px-6 py-32 flex-1 flex flex-col relative z-10">
         <div className="max-w-2xl mb-24">
            <h1 className="text-4xl md:text-6xl font-medium tracking-tighter text-slate-950 mb-6">
              Structural oversight.
            </h1>
            <p className="text-xl text-slate-500 leading-relaxed font-light">
              Consolidate extracurricular initiatives, academic interventions, and vendor operations into a strict, auditable framework.
            </p>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <motion.div whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className="md:col-span-2 group bg-white border border-slate-200 rounded-[2rem] p-8 lg:p-12 hover:border-slate-950 hover:shadow-xl shadow-sm transition-all duration-300 relative overflow-hidden">
              <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center mb-16 group-hover:bg-slate-950 group-hover:border-slate-950 transition-colors duration-300 relative z-10">
                <Target className="w-5 h-5 text-slate-600 group-hover:text-white transition-colors duration-300" />
              </div>
              <h3 className="text-2xl font-medium text-slate-950 mb-3 relative z-10 transition-colors duration-300">Live Telemetry</h3>
              <p className="text-base text-slate-500 leading-relaxed max-w-md relative z-10">
                Monitor live delivery rates, assessment variances, and vendor execution metrics across all academic cohorts simultaneously.
              </p>
            </motion.div>
            ...
            <motion.div whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className="md:col-span-1 group bg-white border border-slate-200 rounded-[2rem] p-8 lg:p-12 hover:border-slate-950 hover:shadow-xl shadow-sm transition-all duration-300 relative overflow-hidden">
              ...
              <h3 className="text-2xl font-medium text-slate-950 mb-3 relative z-10 transition-colors duration-300">Immutable Evidence</h3>
              <p className="text-base text-slate-500 leading-relaxed relative z-10">
                Aggregates mandatory physical proofs for board inspections effortlessly in one central secure vault.
              </p>
            </motion.div>
            ...
            <motion.div whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className="md:col-span-1 group bg-white border border-slate-200 rounded-[2rem] p-8 lg:p-12 hover:border-slate-950 hover:shadow-xl shadow-sm transition-all duration-300 relative overflow-hidden">
              ...
              <h3 className="text-2xl font-medium text-slate-950 mb-3 relative z-10 transition-colors duration-300">Automated Auditing</h3>
              <p className="text-base text-slate-500 leading-relaxed relative z-10">
                Triggers severity alerts automatically for delayed sessions, missing evidence, or low vendor feedback.
              </p>
            </motion.div>
            ...
            <motion.div whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className="md:col-span-2 group bg-white border border-slate-200 rounded-[2rem] p-8 lg:p-12 hover:border-slate-950 hover:shadow-xl shadow-sm transition-all duration-300 relative overflow-hidden">
              ...
              <h3 className="text-2xl font-medium text-slate-950 mb-3 relative z-10 transition-colors duration-300">Diagnostic Remediation</h3>
              <p className="text-base text-slate-500 leading-relaxed max-w-md relative z-10">
                Convert diagnostic exam results into immediate competency bands, automatically highlighting operational groups requiring focused remediation.
              </p>
            </motion.div>
         </div>
      </main>
