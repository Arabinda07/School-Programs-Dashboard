  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-slate-950 font-sans">Evaluation outcomes</h1>
          <p className="text-sm text-slate-500 mt-1 font-light">Diagnostic and post-program evaluation trends, participation, and remediation tracking.</p>
        </div>
      </div>

      {/* KPI Grid */}
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
        <Card className="border-slate-200 shadow-none bg-white col-span-2 md:col-span-1">
          <CardContent className="p-6 flex flex-col justify-center items-center text-center">
            <div className="p-2 bg-indigo-50 text-indigo-600 rounded-full mb-2"><BookOpen className="w-5 h-5" /></div>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Conducted</p>
            <h4 className="text-xl font-bold text-slate-900 mt-0.5">{totalConducted}</h4>
          </CardContent>
        </Card>
        <Card className="border-slate-200 shadow-none bg-white col-span-2 md:col-span-1">
          <CardContent className="p-6 flex flex-col justify-center items-center text-center">
            <div className="p-2 bg-blue-50 text-blue-600 rounded-full mb-2"><Users className="w-5 h-5" /></div>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Target Cohort</p>
            <h4 className="text-xl font-bold text-slate-900 mt-0.5">{totalAssigned}</h4>
          </CardContent>
        </Card>
        <Card className="border-slate-200 shadow-none bg-white col-span-2 md:col-span-1">
          <CardContent className="p-6 flex flex-col justify-center items-center text-center">
            ...
            <h4 className="text-xl font-bold text-slate-900 mt-0.5">{totalCompleted}</h4>
          </CardContent>
        </Card>
        <Card className="border-slate-200 shadow-none bg-white col-span-2 md:col-span-1">
          <CardContent className="p-6 flex flex-col justify-center items-center text-center">
            <div className="p-2 bg-cyan-50 text-cyan-600 rounded-full mb-2"><Target className="w-5 h-5" /></div>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Turnout</p>
            <h4 className="text-xl font-bold text-slate-900 mt-0.5">{Math.round(overallCompletionRate)}%</h4>
          </CardContent>
        </Card>
        <Card className="border-slate-200 shadow-none bg-white col-span-2 md:col-span-1">
          <CardContent className="p-6 flex flex-col justify-center items-center text-center">
             ...
             <h4 className="text-xl font-bold text-slate-900 mt-0.5">{Math.round(averageScore)}%</h4>
          </CardContent>
        </Card>
        <Card className="border-slate-200 shadow-none bg-white col-span-2 md:col-span-1">
          <CardContent className="p-6 flex flex-col justify-center items-center text-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="border-slate-200 shadow-none bg-white lg:col-span-2">
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 py-4">
             <CardTitle className="text-xs flex items-center space-x-2 font-semibold text-slate-950">
               ...
               {enrichedAssessments.slice(0, 5).map((asm: any) => (
                 <div key={asm.id} className="space-y-1">
                    <div className="flex justify-between text-xs font-medium">
                      <span className="text-slate-900">{asm.name} <span className="text-slate-500 font-normal">({asm.className})</span></span>
                      <span className="text-slate-500">Avg {asm.average_score_percent}%</span>
                    </div>
               ...
               <ul className="divide-y divide-rose-100/50">
                 {highRemediation.map((asm: any) => (
                   <li key={asm.id} className="p-3">
                     <p className="text-sm font-medium text-slate-900">{asm.programName}</p>
                 ...
                 {highRemediation.length === 0 && <li className="p-4 text-xs text-slate-500">No classes currently over 40% remediation.</li>}
               </ul>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-0">
               <ul className="divide-y divide-amber-100/50">
                 {lowCompletion.map((asm: any) => (
                   <li key={asm.id} className="p-3">
                     <p className="text-sm font-medium text-slate-900 line-clamp-1">{asm.name}</p>
                 ...
                 {lowCompletion.length === 0 && <li className="p-4 text-xs text-slate-500">Turnout targets met for all assessments.</li>}
               </ul>
            </CardContent>
          </Card>
        </div>
      ...
      <div className="flex flex-wrap gap-4 items-center pt-4">
        ...
        <div className="flex items-center space-x-2 bg-white border border-slate-200 rounded-lg p-1">
          <Stack className="w-4 h-4 text-slate-500 ml-2" />
          <select 
            className="text-sm border-none focus:ring-0 bg-transparent py-1 pr-8 text-slate-700 cursor-pointer"
            value={groupBy}
      ...
      <Card className="border-slate-200 shadow-none bg-white overflow-hidden mt-4">
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50/50 text-slate-500 text-xs uppercase tracking-wider border-b border-slate-100/80">
              <tr>
                <th className="px-6 py-2.5 font-medium text-slate-500">Evaluation name</th>
                <th className="px-6 py-2.5 font-medium hidden md:table-cell text-slate-500">Context</th>
                <th className="px-6 py-2.5 font-medium text-center text-slate-500">Participation</th>
                <th className="px-6 py-2.5 font-medium text-center text-slate-500">Average score</th>
                <th className="px-6 py-2.5 font-medium text-center text-slate-500">Growth</th>
                <th className="px-6 py-2.5 font-medium text-right text-slate-500">Date</th>
              </tr>
            </thead>
                ...
                <React.Fragment key={groupName}>
                  {groupBy !== 'None' && (
                    <tr className="bg-slate-50/80">
                      <td colSpan={6} className="px-6 py-3 font-semibold text-slate-700 text-xs uppercase tracking-wider">
                  ...
                  <tr key={asm.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-sm text-slate-500 mt-1 font-light">Class {asm.className}</div>
                    </td>
                    <td className="px-6 py-4 hidden md:table-cell">
                      <div className="text-slate-900">{asm.programName}</div>
                      <div className="text-sm text-slate-500 mt-1 font-light">{asm.category}</div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <div className="font-medium text-slate-900">{asm.students_participated} / {asm.studentsAssigned}</div>
                      <div className={`text-[10px] mt-1 ${asm.completionRate < 75 ? 'text-amber-600 font-medium' : 'text-slate-500'}`}>
                        {Math.round(asm.completionRate)}% Turnout
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <div className="font-bold text-slate-900">{asm.average_score_percent}%</div>
                      <div className="w-16 h-1 mt-1.5 bg-slate-100 rounded-full mx-auto overflow-hidden">
                    ...
                    <td className="px-6 py-4 text-right">
                       <span className="text-xs text-slate-600">{asm.conducted_date}</span>
                    </td>
                  </tr>
                  ))}
                </React.Fragment>
